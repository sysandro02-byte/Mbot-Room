...
try {
  ...
  await hostRoom.page.waitForFunction(() => {
    ...
    return Boolean(
      overlay
      && overlay.getAttribute('data-mode') === 'browser'
      && overlay.textContent?.includes('Sous-titre automatique participant')
    );
  }, undefined, { timeout: 12_000 });

  const captionRows = await jsonRequest(`/api/meetings/${meeting.id}/captions`, { headers: authHeaders(host.token) });
  assert.equal(captionRows.response.status, 200, JSON.stringify(captionRows.data));
  assert.ok(captionRows.data.some((item) => item.text === 'Sous-titre automatique participant' && item.provider === 'browser-speech'));
