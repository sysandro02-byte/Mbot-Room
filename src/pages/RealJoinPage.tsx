...
export default function RealJoinPage(){
  ...
  const join=async()=>{if(!meeting)return;setLoading(true);setError('');try{const host=Number(meeting.host_id)===Number(user?.id)||Number(meeting.co_host_id||0)===Number(user?.id)||user?.role==='admin';if(host){if(!meeting.is_active){const result=await meetingService.startMeetingAndNotify(meeting.id);const live=result.meeting||{...meeting,is_active:true};navigate(`/reunions/${meeting.id}`,{state:{meeting:live,joinOptions:{mic,camera}}});return;}navigate(`/reunions/${meeting.id}`,{state:{meeting,joinOptions:{mic,camera}}});return;}const numericUserId=Number(user?.id);const result=await meetingService.requestJoin(meeting.id,Number.isFinite(numericUserId)?numericUserId:undefined,password);if(result.status==='requested'){navigate(`/reunions/${meeting.id}/salle-attente`,{state:{meeting,joinOptions:{mic,camera}}});return;}navigate(`/reunions/${meeting.id}`,{state:{meeting,joinOptions:{mic,camera}}});}catch(cause){setError(cause instanceof Error?cause.message:'Impossible de rejoindre la réunion.');}finally{setLoading(false);}};
