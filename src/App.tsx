...
function SimpleInfoPage({ title, description }: { title: string; description: string }) {
  ...
  return (
    <main className="simple-info-page" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closeModal(); }}>
      <section className="simple-info-modal" role="dialog" aria-modal="true" aria-labelledby="simple-info-title">
        <button className="simple-info-close" type="button" aria-label="Fermer" onClick={closeModal}><X size={20} aria-hidden="true" /></button>
